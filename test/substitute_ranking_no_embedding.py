...
def substitute_ranking(row_line, model, tokenizer, hownet, source_sentence, source_word, substitution_words, word_freq_dict, substitution_num):
    MAX = 56065
    ...
    assert len(loss_scores) == len(freq_scores) == len(hownet_scores)
    ...
    all_ranks = [[substitution_word, loss+freq+hownet] for substitution_word, loss, freq, hownet in zip(substitution_words, loss_ranks, freq_ranks, hownet_ranks)]
...
def main():
    ...
    for row_line, source_sentence, source_word, bert_subs, vector_subs, dict_subs, hownet_subs, mix_subs in zip(row_lines, source_sentences, source_words, bert_res, vector_res, dict_res, hownet_res, mix_res):
        ...
        if vector_subs[0] != 'NULL':
            vector_pre_word, vector_ss_sorted = substitute_ranking(row_line, model, tokenizer, hownet, source_sentence, source_word, vector_subs, word_freq_dict, substitution_num)
        else:
            vector_pre_word = 'NULL'
            vector_ss_sorted = ['NULL']
        if dict_subs[0] != 'NULL':
            dict_pre_word, dict_ss_sorted = substitute_ranking(row_line, model, tokenizer, hownet, source_sentence, source_word, dict_subs, word_freq_dict, substitution_num)
        else:
            dict_pre_word = 'NULL'
            dict_ss_sorted = ['NULL']
        if hownet_subs[0] != 'NULL':
            hownet_pre_word, hownet_ss_sorted = substitute_ranking(row_line, model, tokenizer, hownet, source_sentence, source_word, hownet_subs, word_freq_dict, substitution_num)
        else:
