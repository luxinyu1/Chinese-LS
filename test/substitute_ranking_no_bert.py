...
def main():
    ...
    for row_line, source_sentence, source_word, bert_subs, vector_subs, dict_subs, hownet_subs, hybrid_subs in zip(row_lines, source_sentences, source_words, bert_res, vector_res, dict_res, hownet_res, hybrid_res):
        ...
        if hybrid_subs[0] != 'NULL':
            hybrid_pre_word, hybrid_ss_sorted = substitute_ranking(model_word2vector, hownet, source_sentence, source_word, hybrid_subs, word_freq_dict, substitution_num)
        else:
        ...
        save_result(row_line, hybrid_pre_word, hybrid_ss_sorted, './test/data/nobert/hybrid_sr_res_no_bert.csv')
