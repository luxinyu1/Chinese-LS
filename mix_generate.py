...
def read_eval_dataset(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as reader:
        ...
        assert len(sentences) == len(difficult_words)
...
def bert_generate(tokenizer, model, difficult_sentence, difficult_word, max_length):

    len_word = len(difficult_word)
    if len(difficult_sentence) > int((max_length-3) / 2):
        start_index = difficult_sentence.index(difficult_word)
        end_index = start_index + len_word
        difficult_sentence = cut_out_sent(difficult_sentence, start_index, end_index, int((max_length-3) / 2))
    len_sentence = len(difficult_sentence)
    logits, substitutions = get_word_subs(tokenizer, model, difficult_sentence, difficult_word, max_length)
    if len_word == 4:
        idiom_logits, idiom_substitutions = get_idiom_subs(tokenizer, model, difficult_sentence, difficult_word, max_length)
        substitutions.extend(idiom_substitutions)
    if len_word == 2:
        _, one_char_word_substitutions = get_char_subs(tokenizer, model, difficult_sentence, difficult_word, max_length, 2)
        substitutions.extend(one_char_word_substitutions)
    if len_word == 1:
        _, one_char_word_substitutions = get_char_subs(tokenizer, model, difficult_sentence, difficult_word, max_length, 3)
        substitutions.extend(one_char_word_substitutions)
...
def main():
    ...
    for sentence, difficult_word in zip(sentences, difficult_words):
        ...
        if isFound == False:
            substitution_words = bert_generate(tokenizer, model, sentence, difficult_word, max_length)
